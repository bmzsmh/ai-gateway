...
# 将 <YOUR_ADMIN_USERNAME> 替换为管理后台用户名
# 注意：ADMIN_PASSWORD 不在此文件配置，见下文

# 基于模板创建测试配置
cp wrangler-test.toml.example wrangler-test.toml
# 编辑 wrangler-test.toml，将 <YOUR_TEST_KV_NAMESPACE_ID> 替换为测试 KV ID
# 将 <YOUR_ADMIN_USERNAME> 和 <YOUR_ADMIN_PASSWORD> 替换为测试环境凭据
```
...
> 工作流会自动生成 `wrangler.toml`（含 KV 绑定），admin 凭据由 `wrangler secret put` 独立管理，不通过 CI 写入。`OPENCODE_MIRRORS_URL` 使用默认的三个镜像地址，可通过 GitHub Variables 覆盖。
